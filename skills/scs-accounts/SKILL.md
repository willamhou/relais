...
- Optional: if SCS later requires auth, store the `acs_token` once with
  `relais vault store scs-v2 <acs_token>`; relais injects it as `Authorization: Bearer`.
...
relais apis scs-v2                  # the accounts resource + all actions
relais spec scs-v2.accounts.create  # one action's JSON Schema (params/returns)
```
...
- Always confirm the target with `relais spec scs-v2.accounts.<action>` if unsure
