//! Generic credential/secret redaction — **always compiled** (not feature-gated), so
//! non-audit code (e.g. the server error path, PR-D) can sanitise output too.
//!
...
//!    [`crate::types::Credentials`]) are masked wherever they appear, under *any* key
//!    or as a substring of a larger string.
//!
//! Audit-specific metadata (`AuditMeta`) lives in [`crate::audit::redact`], which
...
impl Redactor {
    ...
    /// Mask secrets in a plain string (e.g. an error message).
    ///
    /// **Residual risk (accepted for v1, RQ3):** this is exact-substring masking. A
    /// secret a nested error emits in a *transformed* form (URL-encoded, base64,
    /// truncated) can survive — v1 redaction is denylist + raw-value matching, not
    /// format-aware.
...
/// Return `key`, or a `key#N` variant not yet in `out`, so masking two distinct keys
/// to the same string never silently drops a field (RQ1).
...
/// suffix material from a longer one.
